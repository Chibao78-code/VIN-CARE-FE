...
// trang quan ly lich hen cua nhan vien
const StaffAppointments = () => {
  ...
  //  load data khi trang thay doi statusFilter
  useEffect(() => {
    fetchBookings();
    fetchTechnicians();
  }, [statusFilter]);

  //  refresh technicians khi trang tro lai hoac focus
  useEffect(() => {
  ...
  //  lay danh sach ky thuat vien
  const fetchTechnicians = async () => {
    try {
      const result = await bookingService.getMyTechnicians();
      if (result.success && result.data) {
// hien thi ky thuat vien trong console de kiem tra
  ...
  //  lay danh sach booking tu backend
  const fetchBookings = async () => {
    setLoading(true);
    try {
      let result;
      //  neu co loc trang thai
      if (statusFilter !== "all") {
      ...
      if (result.success && result.data) {
        ...
        if (bookingsList.length === 0) {
          ...
          //  chuyen doi du lieu backend de phu hop voi frontend
        ...
        setAppointments([]);
        // hien thi loi neu khong phai 404
  ...
  // map data trang thai backend sang frontend
  ...
  // chueybn den trang tiep nhan xe
  const handleNavigateToReception = (booking) => {
    console.log("Navigating to reception with booking:", booking);
   // luu tru thong tin booking trong sessionStorage de su dung o trang tiep nhan xe
    const receptionData = {
