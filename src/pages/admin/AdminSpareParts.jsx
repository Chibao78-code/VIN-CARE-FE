...
import { sparePartService } from '../../services/sparePartService';
 //   Admin Spare Parts Management Page
const AdminSpareParts = () => {
  ...
  // fetch danh sach phu tung
  const fetchSpareParts = async () => {
    setLoading(true);
    try {
      //  Goi backend lay danh sach phu tung
      ...
      //  Transform data backend sang frontend
  ...
  // Loc phu tung theo tim kiem va danh muc
  ...
  // Xu ly them phu tung moi
  const handleAddPart = async (e) => {
    e.preventDefault();
    // validation chon trung tam
    if (!formData.centerId) {
      toast.error('Vui lòng chọn trung tâm bảo dưỡng');
      return;
    }
     // validation gia phu tung hop le
    try {
      const newPart = {
      ...
      // Goi backend de tao phu tung moi
  ...
  // Xu ly cap nhat so luong ton kho
  const handleEditPart = async (e) => {
    ...
    try {
      ...
      // Goi backend de cap nhat phu tung
  ...
  const handlePriceUpdate = async (e) => {
    e.preventDefault();
    try {
     // cbi du lieu phu tung moi voi gia moi
      const newPart = {
        partName: selectedPart.partName,
      ...
      // Vo hieu hoa phien ban cu (backend se dat trang thai la DISABLED va so luong la 0)
