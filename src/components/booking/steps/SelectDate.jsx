...
const SelectDate = ({ data, onNext }) => {
  ...
  const formatDate = (date) => {
    const year = date.getFullYear(); // nam
    const month = String(date.getMonth() + 1).padStart(2, '0'); // thang
    const day = String(date.getDate()).padStart(2, '0'); // ngay
    return `${year}-${month}-${day}`;
  };

  // Dat ngay mac dinh la hom nay neu chua co
  const [selectedDate, setSelectedDate] = useState(data.date || formatDate(today));
  const dateInputRef = useRef(null); // tham chieu den input date

  // Tu dong luu ngay vao parent khi thay doi
  React.useEffect(() => {
    if (selectedDate) { // goi ham onNext truyen ngay da chon len buoc tiep theo
  ...
  const formatLongDate = (dateString) => {
    ...
    const date = new Date(dateString);// tao doi tuong date
    const day = date.getDate(); // lay ngay
    const month = date.getMonth() + 1; // lay thang
    const year = date.getFullYear(); // lay nam
  ...
  const getVehicleInfo = () => {
    ...
    if (model && plate) { // neu co ca ten xe va bien so
