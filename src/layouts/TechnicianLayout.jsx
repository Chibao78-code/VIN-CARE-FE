...
import { 
  ...
  FiLogOut, FiBell, FiClock
} from 'react-icons/fi';
import useAuthStore from '../store/authStore';
 // layout cho ky thuat vien
const TechnicianLayout = () => {
  ...
  useEffect(() => {
    if (user?.centerName) {
      ...
      setCenterName('Trung tâm Dịch vụ EV');
    }
  }, [user]);
  ...
  const menuItems = [
    { path: '/technician/dashboard', label: 'Tổng quan', icon: FiGrid },
    { path: '/technician/work-orders', label: 'Công việc', icon: FiClipboard },
    { path: '/technician/history', label: 'Lịch sử', icon: FiClock },
  ];
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center text-white font-bold text-lg">
              {user?.fullName?.charAt(0)?.toUpperCase() || user?.name?.charAt(0)?.toUpperCase() || 'T'}
            </div>
            {isSidebarOpen && (
